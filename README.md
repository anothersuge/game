...
- 后端架构：Spring Boot 3 + Java 21 + Lombok
- 框架：Spring Boot 3 + Spring Data JPA
- 数据库：SQLite
- 前端模板引擎：Thymeleaf（前后端不分离）
- 缓存：Caffeine（用于游戏库、评论和点赞查询优化）

## 重要技术细节

### 查询性能优化
为了提升系统性能，特别是在处理大量游戏和评论数据时，我们实施了多项优化措施：

1. 游戏库查询优化：
   - 游戏库页面现在只显示最近创建的100个游戏，减少数据加载量
   - 为SQLite数据库配置了连接池以提高性能
   - 为Game实体添加了数据库索引以优化查询性能
   - 使用Caffeine缓存游戏库查询结果以提高响应速度

2. 评论查询优化：
   - 解决了N+1查询问题，使用JOIN FETCH优化评论加载
   - 为评论查询添加了Caffeine缓存以减少数据库访问次数

3. 点赞功能优化：
   - 使用Caffeine缓存点赞计数，减少数据库查询
   - 实施点赞记录清理任务，定期清理过期记录
...
2. 点赞记录清理任务：每小时执行一次，清理超过一天的点赞记录，一个IP每天可以对一个评论点赞一次
